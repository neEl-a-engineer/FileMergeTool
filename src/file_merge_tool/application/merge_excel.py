...
def merge_excel(request: MergeRequest) -> MergeResult:
    ...
    normal_sources: list[dict[str, object]] = []
    sensitive_sources: list[dict[str, object]] = []
    warnings: list[str] = []
    skipped_count = 0
    ...
    normal_path = request.output_dir / f"{output_stem}.xlsx"
    sensitive_path = request.output_dir / f"{output_stem}{SENSITIVE_SUFFIX}.xlsx"
...
def _header_lines(
    ...
    return [
        ...
        "Schema: file-merge-tool/excel-merge/v1",
